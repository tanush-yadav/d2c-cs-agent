...
*   `get_orders(first: int, query: str) -> dict`: Retrieves a list of orders for the current customer with query customer_id=CUSTOMER_ID
...
order_agent = Agent(
    ...
    description="Handles all order-related inquiries including order tracking, status updates, and order history. Use for any questions containing 'order', 'tracking', 'shipment', or 'delivery'.",
    tools=[],
)
